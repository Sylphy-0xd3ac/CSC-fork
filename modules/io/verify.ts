...
export async function run(_hazel, core, _hold) {
  ...
  // 判断聊天室名是否合法
  core.verifyChannel = (channel) => /^[a-zA-Z0-9\u4e00-\u9fa5_-]{1,20}$/.test(channel);

  // 判断 trip 是否合法
  core.verifyTrip = (trip) => /^[a-zA-Z0-9+/]{6}$/.test(trip);
}
