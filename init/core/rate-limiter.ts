...
export async function run(hazel, core, hold) {
  ...
  core.checkAddress = function (remoteAddress, score) {
    ...
    hold.rateRecords[remoteAddress].score *= Math.pow(
      2,
    ...
    if (
      hold.rateRecords[remoteAddress].score >= core.config.rateLimiter.limit
    ) {
      return true;
    }
  ...
  core.increaseGlobalRate = function () {
    ...
    if (thisTime - lastRateTime < core.config.rateLimiter.globalTimeRange) {
      hold.perviousRate =
        hold.perviousRate *
          (1 -
            (thisTime - lastRateTime) /
              core.config.rateLimiter.globalTimeRange) +
        1;
    } else {
      hold.perviousRate =
        core.config.rateLimiter.globalTimeRange / (thisTime - lastRateTime);
    }
  ...
  core.getFrequency = function () {
    return (
      hold.perviousRate * (60000 / core.config.rateLimiter.globalTimeRange)
    );
  };
}
